...
require 'scripts/connect.php';
			session_start();
			$query="SELECT * FROM tasktable WHERE worker LIKE '%".$_SESSION['surname']."%' OR 2worker LIKE '%".$_SESSION['surname']."%' OR 3worker LIKE '%".$_SESSION['surname']."%'";
...
if (isset($_POST['search'])) {
	$sql.= ' AND (worker LIKE "%'.$_SESSION['surname'].'%" OR 2worker LIKE "%'.$_SESSION['surname'].'%" OR 3worker LIKE "%'.$_SESSION['surname'].'%")';
...
if (isset($_POST['control'])) {																		//по нажатию кнопки Отобразить заявки на контроле ищем заявки с дедлайноm, равным (сегд число +3 дня)
	...
	$query = "SELECT * FROM tasktable WHERE deadline <=".$findindeadline." AND deadline >=".$findoutdeadline." AND status!='Выполнена' AND worker LIKE '%".$_SESSION['surname']."%' OR 2worker LIKE '%".$_SESSION['surname']."%' OR 3worker LIKE '%".$_SESSION['surname']."%' ORDER BY number DESC";
	$querynum=$query;
	$quantrows=mysql_query($query);
...
{ 
$query = "SELECT * FROM tasktable WHERE worker LIKE '%".$_SESSION['surname']."%' OR 2worker LIKE '%".$_SESSION['surname']."%' OR 3worker LIKE '%".$_SESSION['surname']."%' ORDER BY number DESC LIMIT ".$offset.", ".$limit;	
$querynum = "SELECT * FROM tasktable WHERE worker LIKE '%".$_SESSION['surname']."%' OR 2worker LIKE '%".$_SESSION['surname']."%' OR 3worker LIKE '%".$_SESSION['surname']."%' ORDER BY number DESC";	
echo 'Найдено заявок: '.$numm;
}
...
while ($row = mysql_fetch_array($result)){
				...
				$worker3color=$row["worker_3_check"];	
